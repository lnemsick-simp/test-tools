...
require 'json'
# JiraHelper was shamelessly copied from puppetsync
class JiraHelper
  def initialize(
...
puts _jql
  ...
  def custom_field_id(regex = %r{^story points}i)
  ...
  def issuetype_id(regex = %r{^Sub-task})
...
if $PROGRAM_NAME == __FILE__
  fail('Configuration file not specified') unless ARGV.size != 0
  fail('JIRA_USER must be set') unless ENV.key?('JIRA_USER')
  fail('JIRA_API_TOKEN must be set') unless ENV.key?('JIRA_API_TOKEN')
  ...
  begin
    ...
    opts = config.select { |k, _v| k.to_s =~ %r{^subtask_} }
    components = config['components']
    components.each do |component|
      puts "Ensuring a subtask exists for #{config['parent_issue']} for #{component}"
      ...
      puts "Subtask of #{config['parent_issue']} for #{component} is #{subtask_key}"
    end
  rescue Exception => e
    exit_code = 1
  end
